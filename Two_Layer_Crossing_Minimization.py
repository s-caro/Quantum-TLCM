...
def _add_discrete_constraint(cqm: ConstrainedQuadraticModel, vars: Variables, G: nx.Graph):
  inserted = set()
  for index, (i,k) in enumerate(list(G.edges)[:-1]):
    for (j,n) in list(G.edges)[index+1:]:
...
def _add_transitive_constraint(cqm: ConstrainedQuadraticModel, vars: Variables, G: nx.Graph):
...
if __name__ == '__main__':
  file_edges = open('experimental_dataset.txt','r')
  ...
  for index,line in enumerate(lines[0:]):
    ...
    res = sampler.sample_cqm(cqm,  label="TLCM - hybrid")
